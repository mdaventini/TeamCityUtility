...
<#
	.SYNOPSIS
		Set a TeamCity Parameter on a TeamCity Instance by Locator. If the parameter does not exists, it's created.
	.DESCRIPTION
...
#>
	...
	try {
		...
		$TCResponse = (Invoke-RestMethod -Method Get -Uri $UriInvoke -Credential $TCCredential -Verbose)
		...
		$TCResponse = ( Invoke-RestMethod -Method PUT -Uri $UriInvoke -ContentType "application/json" -Credential $TCCredential -Body $JData -Verbose )
		$ESValue = $TCResponse.property.value
	}
	try {
		#Keep existing data
		if ( "$TCParamValue" -ne "keep" ) {
			if ( "$ESValue" -ne "$TCParamValue" ) {
				...
				$TCResponse = ( Invoke-RestMethod -Method PUT -Uri $UriInvoke/value -Credential $TCCredential -Body $TCParamValue -Verbose )
			} else {
		...
		if ( "$TCParamRaw" -ne "keep" ) {
			if ( "$ESRawValue" -ne "$TCParamRaw" ) {
				...
				$TCResponse = ( Invoke-RestMethod -Method PUT -Uri $UriInvoke/type/rawValue -Credential $TCCredential -Body $TCParamRaw -Verbose ) 
			} else {
